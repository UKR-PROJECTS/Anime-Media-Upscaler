...
class UpscaleWorker(QRunnable):
    ...
    def _extract_frames(self, video_path: str, frames_dir: str):
        ...
        cmd = [
            ffmpeg_path, '-i', video_path, '-q:v', '1', '-pix_fmt', 'rgb24',
    ...
    def _reassemble_video(self, upscaled_dir: str, output_path: str, original_video: str):
        ...
        audio_cmd = [ffmpeg_path, '-i', original_video, '-vn', '-acodec', 'copy', temp_audio]
        subprocess.run(
            audio_cmd, capture_output=True,
            creationflags=subprocess.CREATE_NO_WINDOW if os.name == 'nt' else 0
        )
        reassemble_cmd = [
            ffmpeg_path, '-framerate', str(self.settings.get('fps', 24)),
            '-i', os.path.join(upscaled_dir, 'frame_%06d.png'),
            '-i', temp_audio, '-c:v', 'libx264', '-c:a', 'aac',
            '-pix_fmt', 'yuv420p', '-crf', str(self.settings.get('quality', 18)),
            output_path
        ]
        process = subprocess.run(
            reassemble_cmd, capture_output=True, text=True,
            creationflags=subprocess.CREATE_NO_WINDOW if os.name == 'nt' else 0
        )
        try:
